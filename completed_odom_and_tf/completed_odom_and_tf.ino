...
const double encoder_cpr = 1600;               //Encoder ticks or counts per rotation
...
const double max_speed = 0.2;                 //Max speed in m/s
...
CytronMD motor2(PWM_DIR, 8, 9); // PWM 2 = Pin 9, DIR 2 = Pin 10.
...
void loop() {
  ...
  {                                                                           // enter timed loop
    ...
    if (noCommLoops >= noCommLoopMax) {                   //Stopping if too much time without command
      ...
      motor1.setSpeed(abs(PWM_leftMotor));
    }
    else {                                               //Going backward
      motor1.setSpeed(-abs(PWM_leftMotor));
    }
    ...
    if (noCommLoops >= noCommLoopMax) {                   //Stopping if too much time without command
      ...
      motor2.setSpeed(abs(PWM_rightMotor));
    }
    else {                                                //Going backward
      motor2.setSpeed(-abs(PWM_rightMotor));
    }
